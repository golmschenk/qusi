...
Take a moment and look at the lightcurve - how brightness varies with time. There are high and low frequency components in this image. You will see a large gap somewhere in the lightcurve that lasts for about a day. This is not a transit! This is when the spacraft turned its communications antenna towards Earth and downloaded the data it had stored. There will be a lot of high frequency variability in the signal. This is noise in the signal that arises from several sources. There may be a low-frequency variation that is approximately sinusoidal or has components that appear sinusoidal. This is usually an astrophysical phenomenon - variability of the star that hosts the potential planet or variability of other stars within the photometric aperture used to collect the light. Use the tools along the right side of the lightcurve plot to examine the lightcurve.  In the case of a positive transit dataset you should not expect to immediately see the transit by eye. 
