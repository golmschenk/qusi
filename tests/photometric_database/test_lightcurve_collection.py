...
class TestLightCurveCollection:
    def test_has_necessary_attributes(self):
        ...
        assert hasattr(light_curve_collection, 'load_times_and_magnifications_from_path')
    ...
    def test_generating_a_synthetic_signal_from_a_real_signal_does_not_invert_negative_light_curve_shapes(self):
        light_curve_collection = LightCurveCollection()
        times = np.arange(5, dtype=np.float32)
        unnormalized_positive_light_curve_fluxes = np.array([10, 20, 30, 25, 15], dtype=np.float32)
        normalized_positive_light_curve_fluxes, _ = light_curve_collection.generate_synthetic_signal_from_real_data(
        ...
        normalized_negative_light_curve_fluxes, _ = light_curve_collection.generate_synthetic_signal_from_real_data(
