...
class TestLightCurveDatabase:
    ...
    def test_paths_dataset_from_list_or_generator_factory_can_use_a_list(self, database):
        paths = [Path('a'), Path('b'), Path('c'), Path('d'), Path('e'), Path('f')]
        paths_dataset = module.paths_dataset_from_list_or_generator_factory(paths)
    ...
    def test_paths_dataset_from_list_or_generator_factory_can_use_a_generator_factory(self, database):
        ...
        paths_dataset = module.paths_dataset_from_list_or_generator_factory(generator_factory)
    ...
    def test_normalization_does_not_invert_light_curve_shape_when_there_are_negative_values(self, database):
        ...
        normalized_negative_light_curve_fluxes = module.normalize_on_percentiles(unnormalized_negative_light_curve_fluxes)
    ...
    def test_normalization_brings_values_close_to_negative_one_to_one_range(self, database):
        ...
        normalized_positive_light_curve_fluxes = module.normalize_on_percentiles(unnormalized_positive_light_curve_fluxes)
    ...
    def test_percentile_normalization_gives_exact_results(self, database):
        ...
        normalized_light_curve_fluxes = module.normalize_on_percentiles(unnormalized_light_curve_fluxes)
    ...
    def test_can_normalize_the_flux_channel_of_a_light_curve(self):
        ...
        with patch.object(module, 'normalize_on_percentiles') as mock_normalize_on_percentiles:
            ...
            assert np.array_equal(mock_normalize_on_percentiles.call_args[0][0], light_curve[:, 1])  # Channel 1 should be fluxes.
    ...
    def test_can_normalize_the_flux_channel_of_a_flux_only_light_curve(self):
        ...
        with patch.object(module, 'normalize_on_percentiles') as mock_normalize_on_percentiles:
            mock_normalize_on_percentiles.side_effect = lambda x: x
            database.normalize_fluxes(light_curve=light_curve)
            assert np.array_equal(mock_normalize_on_percentiles.call_args[0][0], light_curve[:, 0])  # Channel 1 should be fluxes.
