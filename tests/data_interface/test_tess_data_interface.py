...
class TestTessDataInterface:
    ...
    def test_can_request_time_series_observations_from_mast_as_pandas_data_frame(self, mock_query_criteria):
        ...
        query_result = tess_data_interface_module.get_all_tess_time_series_observations()
    ...
    def test_can_request_data_products_from_mast_as_pandas_data_frame(self, mock_get_product_list):
        mock_query_result = Table({'a': [1, 2], 'b': [3, 4]})
        mock_get_product_list.return_value = mock_query_result
        fake_observations_data_frame = pd.DataFrame()
        query_result = tess_data_interface_module.get_product_list(fake_observations_data_frame)
        assert isinstance(query_result, pd.DataFrame)
        assert np.array_equal(query_result['a'].values, [1, 2])
    ...
    def test_can_load_fluxes_and_times_from_tess_fits(self, mock_fits_open):
        ...
        fluxes, times = tess_data_interface_module.load_fluxes_and_times_from_fits_file(light_curve_path)
    ...
    def test_can_extract_different_flux_types_from_fits(self, mock_fits_open):
        ...
        mock_fits_open.return_value.__enter__.return_value = hdu_list
        light_curve_path = 'path/to/light_curve'
        sap_fluxes, _ = tess_data_interface_module.load_fluxes_and_times_from_fits_file(light_curve_path,
                                                                                        TessFluxType.SAP)
        pdcsap_fluxes, _ = tess_data_interface_module.load_fluxes_and_times_from_fits_file(light_curve_path,
                                                                                           TessFluxType.PDCSAP)
    ...
    def test_can_load_fluxes_flux_errors_and_times_from_tess_fits(self, mock_fits_open):
        ...
        mock_fits_open.return_value.__enter__.return_value = hdu_list
        light_curve_path = 'path/to/light_curve'
        fluxes, flux_errors, times = tess_data_interface_module.load_fluxes_flux_errors_and_times_from_fits_file(
            light_curve_path)
        mock_fits_open.assert_called_with(light_curve_path)
        assert np.array_equal(fluxes, expected_fluxes)
    ...
    @patch.object(ramjet.data_interface.tess_data_interface.fits, 'open')
    def test_load_fluxes_flux_errors_and_times_from_tess_fits_removing_nans(self, mock_fits_open):
    ...
    @patch.object(ramjet.data_interface.tess_data_interface.fits, 'open')
    def test_load_fluxes_flux_errors_and_times_from_tess_fits_keeping_nans(self, mock_fits_open):
        expected_fluxes = np.array([1, 2, np.nan], dtype=np.float32)
        expected_flux_errors = np.array([4, 5, 6], dtype=np.float32)
        expected_times = np.array([np.nan, 8, 9], dtype=np.float32)
        hdu = Mock(data={'PDCSAP_FLUX': expected_fluxes, 'PDCSAP_FLUX_ERR': expected_flux_errors,
    ...
    def test_can_get_the_coordinates_of_a_target_based_on_tic_id(self, mock_query_criteria):
    ...
    @patch.object(ramjet.data_interface.tess_data_interface.Vizier, 'query_region')
    def test_returns_empty_data_frame_when_no_variable_stars_exist(self, mock_query_region):
