...
class TestTessFfiLightcurveMetadataManager:
    ...
    @patch.object(module, 'dataset_split_from_uuid')
    def test_can_retrieve_training_and_validation_path_generator_from_sql_table(self, mock_dataset_split_generator,
                                                                                metadata_manger):
        test_database = SqliteDatabase(':memory:')
        with test_database.bind_ctx([module.TessFfiLightcurveMetadata], bind_refs=False, bind_backrefs=False):
            test_database.connect()
            module.TessFfiLightcurveMetadata.create_table()
            ...
            mock_dataset_split_generator.side_effect = dataset_splits
            with patch.object(module, 'metadatabase_uuid') as mock_metadatabase_uuid:
                mock_metadatabase_uuid.side_effect = [f'{index:02}' for index in range(20)]
                metadata_manger.insert_multiple_rows_from_paths_into_database(lightcurve_paths)
