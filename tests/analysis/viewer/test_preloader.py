...
class TestPreloader:
    ...
    async def test_loading_view_entity_at_index_as_current_passes_correct_row(self):
        preloader = Preloader()
        preloader.identifier_data_frame = pd.DataFrame({'light_curve_path': ['a.fits', 'b.fits', 'c.fits']})
        ...
        with patch.object(module.ViewEntity, 'from_identifier_data_frame_row') as mock_view_entity_factory:
            await preloader.load_view_entity_at_index_as_current(index)

            assert mock_view_entity_factory.call_args.args[0].equals(preloader.identifier_data_frame.iloc[index])

    @pytest.mark.asyncio
    async def test_loading_next_view_entities_loads_starting_from_last_in_next_deque(self):
        preloader = Preloader()
        preloader.identifier_data_frame = pd.DataFrame({'light_curve_path': ['a.fits', 'b.fits', 'c.fits', 'd.fits']})
        stub_view_entity_dictionary = {'a.fits': Mock(index=0), 'b.fits': Mock(index=1), 'c.fits': Mock(index=2),
                                       'd.fits': Mock(index=3)}
        ...
        with patch.object(module.ViewEntity, 'from_identifier_data_frame_row') as stub_view_entity_factory:
    ...
    @pytest.mark.asyncio
    async def test_loading_next_view_entities_loads_from_current_when_next_deque_is_empty(self):
        preloader = Preloader()
        preloader.identifier_data_frame = pd.DataFrame({'light_curve_path': ['a.fits', 'b.fits', 'c.fits', 'd.fits']})
        stub_view_entity_dictionary = {'a.fits': Mock(index=0), 'b.fits': Mock(index=1), 'c.fits': Mock(index=2),
                                       'd.fits': Mock(index=3)}
    ...
    @pytest.mark.asyncio
    async def test_loading_previous_view_entities_loads_from_first_in_previous_deque(self):
        preloader = Preloader()
        preloader.identifier_data_frame = pd.DataFrame({'light_curve_path': ['a.fits', 'b.fits', 'c.fits', 'd.fits']})
        stub_view_entity_dictionary = {'a.fits': Mock(index=0), 'b.fits': Mock(index=1), 'c.fits': Mock(index=2),
                                       'd.fits': Mock(index=3)}
    ...
    async def test_factory_from_csv_path_loads_view_entities(self):
        identifier_data_frame = pd.DataFrame({'light_curve_path': ['a.fits', 'b.fits', 'c.fits', 'd.fits']})
        stub_view_entity_dictionary = {'a.fits': Mock(index=0), 'b.fits': Mock(index=1), 'c.fits': Mock(index=2),
                                       'd.fits': Mock(index=3)}
        def load_view_entity_side_effect(identifier_row):
            return stub_view_entity_dictionary[identifier_row['light_curve_path']]
        ...
        with patch.object(module.ViewEntity, 'from_identifier_data_frame_row') as stub_view_entity_factory, \
                patch.object(module.pd, 'read_csv') as stub_read_csv:
